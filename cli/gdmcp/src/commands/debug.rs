...
pub fn run(client: &ApiClient, command: DebugCommand) -> Result<Value, CliError> {
    match command {
        DebugCommand::Logs { level, limit, out } => {
            ...
            if let Some(level) = level { arguments["level"] = Value::String(level); }
            let value = call(client, "get_editor_logs", arguments, false, false, None, limit, None)?;
            if let Some(path) = out { return write_output(&path, &value); }
            Ok(value)
        }
