...
def main() -> int:
    ...
    try:
        ...
        if "godot://editor/script_summary" not in resource_uris:
            raise AssertionError(f"Missing godot://editor/script_summary in resources/list: {sorted(resource_uris)}")

        tool_summary = tool_call("get_editor_open_script_summary", {}, request_id=2)
        resource_summary = resource_read("godot://editor/script_summary", request_id=3)
        ...
        keys_to_match = (
            ...
            "current_editor_breakpoint_count",
            "open_script_paths",
            "open_script_types",
            "open_script_count",
            "open_script_editor_types",
            "open_script_editor_count",
            "breakpoints",
            "breakpoint_count",
        )
        ...
        print("editor script summary resource flow verified")
        return 0
    finally:
        process.terminate()
        try:
